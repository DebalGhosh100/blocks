...
class StateManager:
    ...
    def __init__(self, initial_directory: str = None, initial_environment: Dict[str, str] = None):
        ...
        # Track which oversized env vars have already triggered a warning
        # (used by env_guard.warn_oversized to avoid log spam).
        self._oversized_warned = set()
    ...
    def parse_and_apply_environment(self, export_output: str):
        ...
        env_guard is consulted to skip a denylist of known-noisy bitbake/oe
        internals, drop oversized values, and enforce an overall
        environment-size budget so that envp cannot exceed ARG_MAX and
        cause execve to fail with [Errno 7] Argument list too long.
        ...
        for line in export_output.split('\n'):
            ...
            if match:
                ...
                if env_guard.is_denylisted_env_var(var_name):
                    continue
                
                # Skip values that exceed the per-variable byte cap
                if env_guard.is_oversized_value(var_value):
                    env_guard.warn_oversized(
                        var_name, var_value, self._oversized_warned
                    )
        ...
        # Enforce overall env-size budget after merge.
        env_guard.enforce_env_size_budget(self.environment)
