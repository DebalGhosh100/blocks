...
echo "=== Handling parameters directory merge ==="
# Handle parameters directory merging before copying other files
if [ -d "temp-clone/parameters" ]; then
    if [ -d "parameters" ]; then
        echo "Merging parameters directories (temp-clone/parameters will override existing files)"
        # Copy/overwrite files from temp-clone/parameters to current parameters
        cp -rf temp-clone/parameters/* parameters/ 2>/dev/null || true
        # Remove parameters from temp-clone to avoid duplication during main copy
        rm -rf temp-clone/parameters
    else
        echo "Creating parameters directory from temp-clone"
        # Parameters doesn't exist locally, will be copied during main copy
    fi
else
    echo "No parameters directory in temp-clone"
fi
...
curl -sSL https://raw.githubusercontent.com/DebalGhosh100/blocks/main/subroutines/run_blocks.sh | bash -s "${NEW_MAIN_NAME}_temp" "${NEW_MAIN_NAME}.yaml" parameters 2>&1
